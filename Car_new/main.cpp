...
class Engine
{
	...
	double default_consumption_per_second; //расход в сек
	bool is_started;
public:
	...
	void speed_consumption_dependency(double speed)
	{
		...
		else if (speed < 100)consumption_per_second = default_consumption_per_second * 10;
...
class Car
{
	...
	struct Control
	{
		std::thread panel_tread;
		std::thread engine_idle_tread; //холостой ход двигателя
...
public:
	...
	void get_in()
	{
		driver_inside = true;
		//panel();
		control.panel_tread = std::thread(&Car::panel, this);
	}
	void get_out()
	{
		driver_inside = false;
		system("CLS");
		if (control.panel_tread.joinable())control.panel_tread.join();
	...
	void start_engine()
	{
		...
		{
			engine.start();
			control.engine_idle_tread = std::thread(&Car::engine_idle, this);
		}
	}
	void stop_engine()
	{
		engine.stop();
		if (control.engine_idle_tread.joinable())
		{
			control.engine_idle_tread.join();
		}
	}
	...
	void control_car()
	{
		...
		do
		{
			...
			if (!engine.started() && control.engine_idle_tread.joinable())
				control.engine_idle_tread.join();
	...
	void panel()const
	{
		while (driver_inside)
		{
			...
			cout << "Consupmtion per second: " << engine.get_consumtion_per_second() << " litters.\n";
